...
// acquire load of an immutable frame plus one map find, and a write copies the
// frame, applies its change and publishes the copy. A reader that already
// loaded a frame keeps it alive, so a concurrent publish cannot invalidate an
// in-flight lookup. The registry adds no locking beyond that: each tenant
// synchronizes its own containers.
//
...
// be short, and it must not re-enter the registry.
//
